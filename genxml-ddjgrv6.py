...
def section_browsing(xc, num_decks):
    ...
    xc.comment(il, "control", "**")
    xc.comment(
        il, "control", "** Browsing functionality: top middle section with the jogwhhel"
    )
    xc.comment(il, "control", "**")
    xc.comment(il, "control", "-->")
    ...
    xc.kv["description"] = (
        "B1 BROWSE - press - Move cursor between track list and tree view"
    )
    ...
    xc.kv["description"] = (
        "B1 BROWSE +SHIFT - press - Move cursor between track list and tree view"
    )
...
def section_deck(xc, num_decks):
    ...
    for i in range(num_decks):
        xc.kv["group"] = f"[Channel{i + 1}]"
        xc.kv["description"] = gen_description(
            "D1", "shift Play/Pause", i + 1, "reverse playback"
        )
        xc.kv["status"] = hexfmt(int("0x90", 16) + i)
        xc.print(il, "control", entries)
    for i in range(num_decks):
    ...
    for i in range(num_decks):
        xc.kv["group"] = f"[Channel{i + 1}]"
        xc.kv["description"] = gen_description(
            "D2", "set/call cue, back cue", i + 1, "normal"
        )
        xc.kv["status"] = hexfmt(int("0x90", 16) + i)
        xc.print(il, "control", entries)
    ...
    for i in range(num_decks):
        xc.kv["group"] = f"[Channel{i + 1}]"
        xc.kv["description"] = gen_description(
            "D2", "jump to track start", i + 1, "normal"
        )
        xc.kv["status"] = hexfmt(int("0x90", 16) + i)
        xc.print(il, "control", entries)
    for i in range(num_decks):
    ...
    for i in range(num_decks):
        xc.kv["group"] = f"[Channel{i + 1}]"
        xc.kv["description"] = gen_description(
            "D4", "Jog Platter Vinyl mode ON", i + 1, "scratch"
        )
    ...
    for i in range(num_decks):
        xc.kv["group"] = f"[Channel{i + 1}]"
        xc.kv["description"] = gen_description(
            "D4", "Jog Platter Vinyl mode OFF", i + 1, "pitch bend"
        )
        xc.kv["status"] = hexfmt(int("0xB0", 16) + i)
        xc.print(il, "control", entries)
    ...
    for i in range(num_decks):
        xc.kv["group"] = f"[Channel{i + 1}]"
        xc.kv["description"] = gen_description(
            "D4", "Jog Platter touch", i + 1, "enable on touch / disable on release"
        )
        xc.kv["status"] = hexfmt(int("0x90", 16) + i)
        xc.print(il, "control", entries)
    ...
    for i in range(num_decks):
        ...
        xc.kv["description"] = gen_description(
            "D4",
            "Jog Platter touch scratch / pitch bend",
            i + 1,
            "enable on touch / disable on release high speed",
        )
        xc.kv["status"] = hexfmt(int("0x90", 16) + i)
        xc.print(il, "control", entries)
    ...
    for i in range(num_decks):
        xc.kv["group"] = f"[Channel{i + 1}]"
        xc.kv["description"] = gen_description(
            "D7", "LOOP IN/4 BEAT", i + 1, "press, set loop in"
        )
        xc.kv["status"] = hexfmt(int("0x90", 16) + i)
        xc.print(il, "control", entries)
    ...
    for i in range(num_decks):
        xc.kv["group"] = f"[Channel{i + 1}]"
        xc.kv["description"] = gen_description(
            "D7", "LOOP IN/4 BEAT", i + 1, "press, set loop out"
        )
        xc.kv["status"] = hexfmt(int("0x90", 16) + i)
        xc.print(il, "control", entries)
    ...
    for i in range(num_decks):
        xc.kv["group"] = f"[Channel{i + 1}]"
        xc.kv["description"] = gen_description(
            "D7", "RELOOP/EXIT", i + 1, "loop off, loop on"
        )
        xc.kv["status"] = hexfmt(int("0x90", 16) + i)
        xc.print(il, "control", entries)
    ...
    for i in range(num_decks):
        xc.kv["group"] = f"[Channel{i + 1}]"
        xc.kv["description"] = gen_description(
            "D7", "RELOOP/EXIT + shift", i + 1, "reloop and  stop"
        )
        xc.kv["status"] = hexfmt(int("0x90", 16) + i)
        xc.print(il, "control", entries)
    ...
    for i in range(num_decks):
        xc.kv["group"] = f"[Channel{i + 1}]"
        xc.kv["description"] = gen_description(
            "D7", "shift LOOP in", i + 1, "adjust loop in"
        )
        xc.kv["status"] = hexfmt(int("0x90", 16) + i)
        xc.print(il, "control", entries)
    ...
    for i in range(num_decks):
        xc.kv["group"] = f"[Channel{i + 1}]"
        xc.kv["description"] = gen_description(
            "D7", "shift LOOP out", i + 1, "adjust loop out"
        )
        xc.kv["status"] = hexfmt(int("0x90", 16) + i)
        xc.print(il, "control", entries)
...
def section_mixer(xc, num_decks):
    ...
    # Color FX (Sound Color FX) knob - one per channel, all on status 0xB6.
    # CC numbers observed from hardware (14-bit pairs, MSB+0x20=LSB):
    #   CH1: MSB=0x17, CH2: MSB=0x18, CH3: MSB=0x19, CH4: MSB=0x1A
    # Formula: MSB = 0x17 + i  (i=0 for CH1)
    xc.kv["key"] = "super1"
    xc.kv["status"] = "0xB6"
    xc.kv["options"] = "<FourteenBitCC/>"
    for i in range(num_decks):
        xc.kv["description"] = f"Color FX knob - Channel {i + 1}"
        xc.kv["group"] = f"[QuickEffectRack1_[Channel{i + 1}]]"
        xc.kv["midino"] = hexfmt(0x17 + i)
        xc.print(il, "control", entries)
    ...
    xc.kv["description"] = (
        "F6 BEAT FX CH SELECT - Select target deck, samplers (SP) or master"
    )
    xc.kv["key"] = f"{prefix}.fxChannelSelect"
    xc.kv["options"] = "<Script-Binding/>"
    xc.kv["status"] = "0x94"
    for i in range(12):
